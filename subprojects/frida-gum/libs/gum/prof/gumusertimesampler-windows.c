...
gum_user_time_sampler_constructed (GObject * object)
{
  ...
  self->thread =
      OpenThread (THREAD_QUERY_LIMITED_INFORMATION, FALSE, self->thread_id);
}
...
gum_user_time_sampler_sample (GumSampler * sampler)
{
  ...
  if (!GetThreadTimes (self->thread, &creation_time, &exit_time, &kernel_time,
        &user_time))
  {
    goto beach;
  }

  user_time_usec = ((((GumSample) user_time.dwHighDateTime) << 32) +
      user_time.dwLowDateTime) / 10;

beach:
  return user_time_usec;
}
