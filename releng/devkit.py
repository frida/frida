...
def generate_devkit(kit, host, variant, output_dir):
    ...
    (extra_ldflags, thirdparty_symbol_mappings) = generate_library(package, frida_root, host, variant, output_dir, library_filename)

    umbrella_header_path = compute_umbrella_header_path(frida_root, host, variant, package, umbrella_header)
    ...
    header = generate_header(package, frida_root, host, kit, variant, umbrella_header_path, thirdparty_symbol_mappings)
    with codecs.open(os.path.join(output_dir, header_filename), "w", 'utf-8') as f:
        f.write(header)

    example_filename = kit + "-example.c"
    example = generate_example(example_filename, package, frida_root, host, kit, variant, extra_ldflags)
...
def generate_header(package, frida_root, host, kit, variant, umbrella_header_path, thirdparty_symbol_mappings):
    if platform.system() == 'Windows':
        ...
        rc = env_rc(frida_root, host, variant)
...
def generate_library(package, frida_root, host, variant, output_dir, library_filename):
    if platform.system() == 'Windows':
        return generate_library_windows(package, frida_root, host, variant, output_dir, library_filename)
    else:
        return generate_library_unix(package, frida_root, host, variant, output_dir, library_filename)

def generate_library_windows(package, frida_root, host, variant, output_dir, library_filename):
    glib = [
...
def generate_library_unix(package, frida_root, host, variant, output_dir, library_filename):
...
def generate_example(filename, package, frida_root, host, kit, variant, extra_ldflags):
    if platform.system() == 'Windows':
        os_flavor = "windows"
    else:
        os_flavor = "unix"
...
def env_rc(frida_root, host, variant):
    return os.path.join(frida_root, "build", "frida{}-env-{}.rc".format(variant, host))
...
def compute_umbrella_header_path(frida_root, host, variant, package, umbrella_header):
    if platform.system() == 'Windows':
        if package == "frida-gum-1.0":
        ...
        return os.path.join(frida_root, "build", "frida" + variant + "-" + host, "include", *umbrella_header)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--thin", help='build with thin variant', action="store_true")
    ...
    generate_devkit(kit, host, variant, outdir)
