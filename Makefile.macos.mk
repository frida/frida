...
gum-macos-intel: build/frida-macos-x86/lib/pkgconfig/frida-gum-1.0.pc build/frida-macos-x86_64/lib/pkgconfig/frida-gum-1.0.pc
gum-macos-thin: build/frida_thin-macos-$(build_arch)/lib/pkgconfig/frida-gum-1.0.pc ##@gum Build for macOS without cross-arch support
...
check-gum-macos-thin: build/frida_thin-macos-$(build_arch)/lib/pkgconfig/frida-gum-1.0.pc ##@gum Run tests for macOS without cross-arch support
	build/tmp_thin-macos-$(build_arch)/frida-gum/tests/gum-tests $(test_args)
...
core-macos-intel: build/.core-macos-stamp-frida-macos-x86 build/.core-macos-stamp-frida-macos-x86_64
core-macos-thin: build/.core-macos-stamp-frida_thin-macos-$(build_arch) ##@core Build for macOS without cross-arch support
core-ios: build/.core-ios-stamp-frida-ios-arm build/.core-ios-stamp-frida-ios-arm64 build/.core-ios-stamp-frida-ios-arm64e ##@core Build for iOS
...
build/tmp-macos-arm64e/frida-core/.frida-ninja-stamp: build/.frida-core-submodule-stamp build/frida-macos-arm64e/lib/pkgconfig/frida-gum-1.0.pc
	...
	@touch $@
build/tmp-macos-x86/frida-core/.frida-ninja-stamp: build/.frida-core-submodule-stamp build/frida-macos-x86/lib/pkgconfig/frida-gum-1.0.pc
	. build/frida-meson-env-macos-x86.rc; \
	builddir=$(@D); \
	if [ ! -f $$builddir/build.ninja ]; then \
		$(MESON) \
			--cross-file build/frida-macos-x86.txt \
			--prefix $(FRIDA)/build/frida-macos-x86 \
			$(frida_core_flags) \
			...
			-Dagent_legacy=$(FRIDA)/build/tmp-macos-x86/frida-core/lib/agent/frida-agent.dylib \
			frida-core $$builddir || exit 1; \
	fi
	@touch $@
...
build/tmp-ios-x86_64/frida-core/.frida-ninja-stamp: build/.frida-core-submodule-stamp build/frida-ios-x86_64/lib/pkgconfig/frida-gum-1.0.pc
	...
	if [ ! -f $$builddir/build.ninja ]; then \
		$(MESON) \
			...
			-Dagent_legacy=$(FRIDA)/build/tmp-ios-x86/frida-core/lib/agent/frida-agent.dylib \
			frida-core $$builddir || exit 1; \
	fi
	@touch $@
...
build/tmp-ios-arm64/frida-core/.frida-ninja-stamp: build/.frida-core-submodule-stamp build/frida-ios-arm64/lib/pkgconfig/frida-gum-1.0.pc
	...
	if [ ! -f $$builddir/build.ninja ]; then \
		$(MESON) \
			...
			-Dagent_legacy=$(FRIDA)/build/tmp-ios-arm/frida-core/lib/agent/frida-agent.dylib \
			frida-core $$builddir || exit 1; \
	fi
	@touch $@
...
build/frida-macos-x86/lib/pkgconfig/frida-core-1.0.pc: build/tmp-macos-x86/frida-core/.frida-agent-stamp build/tmp-macos-x86_64/frida-core/.frida-helper-and-agent-stamp
	@rm -f build/tmp-macos-x86/frida-core/src/frida-data-{helper,agent}*
	. build/frida-meson-env-macos-x86.rc && $(NINJA) -C build/tmp-macos-x86/frida-core install
	@touch $@
build/frida-macos-x86_64/lib/pkgconfig/frida-core-1.0.pc: build/tmp-macos-x86/frida-core/.frida-agent-stamp build/tmp-macos-x86_64/frida-core/.frida-helper-and-agent-stamp
...
build/frida-macos-universal/lib/frida-gadget.dylib: build/.core-macos-stamp-frida-macos-x86 build/.core-macos-stamp-frida-macos-x86_64 build/.core-macos-stamp-frida-macos-arm64 build/.core-macos-stamp-frida-macos-arm64e
	@mkdir -p $(@D)
...
check-python-macos-thin: python-macos-thin ##@python Test Python bindings for macOS without cross-arch support
	export PYTHONPATH="$(shell pwd)/build/frida_thin-macos-$(build_arch)/lib/$(PYTHON_NAME)/site-packages" \
		&& cd frida-python \
		&& $(PYTHON) -m unittest discover


node-macos: build/frida-macos-$(build_arch)/lib/node_modules/frida ##@node Build Node.js bindings for macOS
node-macos-thin: build/frida_thin-macos-$(build_arch)/lib/node_modules/frida ##@node Build Node.js bindings for macOS without cross-arch support
...
tools-macos-thin: build/tmp_thin-macos-$(build_arch)/frida-tools-$(PYTHON_NAME)/.frida-stamp ##@tools Build CLI tools for macOS without cross-arch support
